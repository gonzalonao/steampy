...
from steampy.models import DEFAULT_HEADERS, Asset, GameOptions, SteamUrl, TradeOfferState
...
class AsyncClient:
    def __init__(
        ...
        self.async_session = aiohttp.ClientSession(headers=DEFAULT_HEADERS)
        ...
        if proxies:
            # Use the first proxy for the sync session (used by cancel_buy_order, etc.)
            first_proxy = proxies[0] if isinstance(proxies, list) else proxies
            self.set_proxies(first_proxy)
        ...
        self.async_market = AsyncMarket(self.async_session, self._session)

        self._access_token = None

        self.set_login_cookies(login_cookies)
        self.async_set_login_cookies(login_cookies)
    ...
    async def close(self):
        await self.async_session.close()  # Ensure the session is properly closed


    def async_set_login_cookies(self, cookies: dict) -> None:
        for name, value in cookies.items():
            self.async_session.cookie_jar.update_cookies({name: value})
        self.was_login_executed = True
        self.async_market._set_login_executed(self.steam_guard, self._get_session_id())
